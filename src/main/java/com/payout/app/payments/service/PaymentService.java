...
public class PaymentService {
    ...
    public PaymentResponse createPayment(User user, Long taskId) {
        ...
        String mockTxId = UUID.randomUUID().toString().substring(0, 8).toUpperCase();

        Payment payment = Payment.builder()
                .task(task)
                .contract(task.getContract())
                .contractor(task.getAssignedTo())
                .amount(task.getBudget())
                .status(PaymentStatus.PAID) // на MVP сразу PAID
    ...
    public List<PaymentResponse> listForUser(User user) {
        if (user.getRole() == UserRole.CONTRACTOR) {
            return paymentRepository.findByContractorId(user.getId())
                    .stream().map(this::toResponse).toList();
        }

        return paymentRepository.findAll()
                .stream()
                .filter(p -> p.getTask().getCompany().getId().equals(user.getCompany().getId()))
                .map(this::toResponse)
                .toList();
